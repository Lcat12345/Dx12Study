...
- [ ] 멀티 머티리얼(Submesh) — *트리거: 에셋 브라우저에 여러 텍스처를 쓰는 실제 모델이 들어와 깨져 보이는 순간.* OBJ `usemtl` 파싱, Mesh를 Submesh(인덱스 구간 + 머티리얼) 목록으로 재구성. 데이터 모델 변경이라 무한정 미루면 비용이 커지는 종류 — 늦어도 glTF 도입(Phase 13 스켈레탈 애니메이션) 때는 함께 한다
- [ ] 씬 계층 구조(Transform 부모-자식) — *트리거: 여러 엔티티를 한 덩어리로 움직이고 싶어지는 순간* (예: 머티리얼별로 쪼갠 모델을 엔티티 여러 개로 배치했을 때)
- [ ] 메시 바운딩 박스(AABB) — *트리거: 뷰포트에서 기존 엔티티를 클릭으로 고르고 싶어지는 순간 — 사실상 곧.* 로드 시점에 정점 범위를 구해 Mesh에 보관. **하나의 데이터로 두 가지가 풀린다**: 레이-AABB 교차(클릭 픽킹)와 배치 시 바닥에 정확히 얹기(지금은 원점이 중심이라 반쯤 파묻힌다) — 10.4에서 둘 다 보류됨
...
- [ ] 삼각형 단위 픽킹 (또는 겹친 후보 순환 선택) — *트리거: 큰 메시의 AABB가 그 안의 오브젝트를 가려 클릭으로 못 고르는 상황이 실제로 작업을 방해할 때.* 11.0에서 확인됨: `Laevat`의 약 21 단위 상자가 `Sphere`와 `Torus`를 통째로 삼켜, 데모 씬 어디를 클릭해도 Laevat이 선택된다. 최근접 규칙 자체는 정확하고 뾰족한 모델에 상자를 씌운 결과다. 해결은 삼각형 교차이거나, 같은 지점을 다시 클릭하면 다음 후보로 넘어가는 순환 선택 — 후자가 훨씬 싸다
