...
- 멀티스레드 커맨드 기록, Bindless (Phase 13에서)
...
#### 덤으로 잡은 Phase 8 버그 — OBJ 감김 방향

사용자가 "도넛 안쪽 글씨가 왜 보이냐"고 물어 드러났다. **Phase 5부터 계속
틀려 있었다.**

OBJ는 오른손 좌표계라 로더가 Z를 음수화하는데, **Z 음수화는 거울 변환이고
거울은 삼각형 방향을 뒤집는다.** Phase 8에 "손잡이가 바뀌면 앞면 판정도
따라 바뀌어 알아서 맞는다"고 적어두었지만 사실이 아니었다. 인덱스 순서를
같이 뒤집지 않으면 뒷면 컬링이 **바깥면을 잘라내고 안쪽 면을 남긴다**.

증상이 미묘해서 오래 살아남았다 — 구는 실루엣이 같아서 티가 안 나고,
토러스만 구멍으로 안쪽 벽이 보였다. 확인 방법:

| 검사 | 수정 전 | 수정 후 |
|---|---|---|
| `cross(e1,e2)` 와 저장된 노멀 일치 (토러스 1920면) | 0 / 1920 | **1920 / 1920** |
| 절차적 큐브 (거울 변환 없음) | 일치 | 일치 |

`CULL_MODE_FRONT`로 뒤집어 렌더링해보니 결정적이었다 — **바닥이 사라지고**
큐브 글씨가 거울상이 되는데(절차적 메시가 뒤집힘) **토러스는 오히려 멀쩡해졌다.**
두 종류의 메시가 서로 반대 규약을 쓰고 있었다는 뜻.

구의 불일치 64건은 극점의 축퇴 삼각형(면적 0)이다. 슬라이스 32 × 극 2 = 64로
정확히 맞아떨어진다.

**교훈**: 좌표계 변환에서 위치만 옮기고 감김 방향을 잊기 쉽다. 그리고 이런
버그는 "그럴듯한 그림"을 만들어내서 눈으로는 몇 단계씩 지나가 버린다.
