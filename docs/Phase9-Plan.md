...
| 단계 | 주제 | 핵심 학습 | 규모(예상) | 태그 |
|------|------|-----------|-----------|------|
| 9.1 | Frame Resources + 티어링 측정 | 프레임 파이프라이닝, fence 심화 | 중 | - |
| 9.2 | Renderer 클래스화 + Engine/Game 분리 | 수명 관리, 소유권, 엔진/게임 경계 | 대 | - |
| 9.3 | Descriptor 할당자 | 디스크립터 힙 운영 전략 | 소 | - |
| 9.4 | 리소스 매니저 | 핸들 기반 자원 관리, 캐싱 | 중 | - |
| 9.5 | ECS | 조합(composition) 기반 설계 | 대 | - |
| 9.6 | ImGui 통합 | 외부 라이브러리 통합, 디버그 UI | 중 | v1.0 |
...
**작업 항목**
- [ ] 측정 먼저: `ALLOW_TEARING` 지원 확인 → 스왑체인 플래그 → `Present(0, ALLOW_TEARING)` 토글(예: V키). 개선 전 CPU 프레임타임을 타이틀바로 기록해 둔다 — **개선을 숫자로 확인하기 위한 사전 작업**
- [ ] FrameResource 단위 정의: 프레임마다 커맨드 할당자 1개 + Object CB 슬롯들 + Pass CB 1개 + fence 값
- [ ] 인플라이트 프레임 수 `kFramesInFlight = 2`로 링 순환
- [ ] 프레임 시작: 이 FrameResource의 fence 값이 완료됐는지만 대기 (완전 플러시 아님)
- [ ] `WaitForGpu()`는 리사이즈·종료·텍스처 업로드 전용으로 강등
- [ ] 개선 후 프레임타임 재측정, 전후 비교 기록
...
- [ ] `wWinMain`이 10줄 내외
- [ ] 매 프레임 완전 플러시 없음 (프레임타임 개선 수치 기록됨)
