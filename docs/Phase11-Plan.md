...
| RT 프레임별 폐기 큐 | 대기 — 11.7 측정에서 resize 프레임이 약 6.05ms → 13.9ms로 상승했지만 최대 15.64ms로 60 Hz 예산 안이며 입력 손실은 없었음 | 고주사율에서 splitter drag의 약 70 fps가 실제 편집을 방해하는지 재확인 |
...
- 애니메이션과 광원 이동을 정지한 같은 820×501 장면을 별도 프로세스에서 1x/4x로
  캡처했다. UI가 가리지 않는 270,540픽셀 중 4,723픽셀(1.7458%)이 달랐고 최대
  채널 차이는 119였다. 차이는 토러스 외곽·내곽과 바닥 대각선 같은 기하 경계에
  집중됐으며 4배 nearest 확대에서 4x 쪽의 중간 coverage 색과 완화된 계단을 확인했다.
- 실제 지원 장치에서 4x와 Off(1x) 화면을 각각 확인했다. 1x↔4x 4회 반복 전환은
  종료 코드 0이었고, 4x 화면의 Debug Layer는 **0 messages**였다.
- Scene 창을 접은 219프레임 동안 viewport 요청은 0×0으로 유지되어 기존 target을
  보존했고, 펼친 첫 프레임에 820×501로 복구됐다. 4x 32회, 1x 33회의 실제 target
  resize도 모두 정상 출력과 종료 코드 0으로 끝났다.
- 4x 미지원을 임시로 강제한 독립 실행에서 632프레임 전부 1x로 기록됐고, 비활성화된
  MSAA UI와 정상 Scene Viewport, 종료 코드 0을 확인했다.
- 연속 resize frame time 측정 결과는 다음과 같다. 리사이즈 직후 다음 프레임의
  `dt`를 기록했으며 시작·모드 전환 프레임은 통계에서 제외했다.

  | 경로 | 안정 시 median / p95 | resize median / p95 / max | 실제 크기 변경 |
  |------|----------------------|-----------------------------|----------------|
  | 4x | 6.0529 / 6.4045 ms | 13.9484 / 14.5887 / 15.6394 ms | 32회 |
  | 1x | 6.0430 / 6.6497 ms | 13.8347 / 14.6941 / 15.4581 ms | 33회 |

  두 모드가 거의 같아 spike의 주원인은 MSAA 메모리량보다 기존 `WaitForGpu()` full
  flush다. 약 2.3배 상승하지만 최대값은 16.67ms(60 Hz) 안이고 drag 입력 손실도
  없었으므로 폐기 큐 트리거는 대기로 유지한다. 다만 165 Hz 기준으로는 resize 중
  약 70 fps가 되므로 고주사율 편집에서 거슬리면 즉시 재발화한다.
- Debug/Release x64 빌드는 경고 0, 오류 0이다.
