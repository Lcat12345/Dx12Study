...
|------|----------------------|-------------|
| 메시 바운딩 박스(AABB) | **발화** — 클릭 선택과 바닥 배치 문제가 모두 존재 | 11.0에서 즉시 구현 |
| 멀티 머티리얼(Submesh) | 대기 | 11.3 검증 모델의 `mtllib`/`usemtl` 확인 직후 |
...
|------|------|------|-----------|-----------|------|
| 11.0 | 예정 | 메시 AABB + 기존 엔티티 클릭 선택 | 로컬/월드 공간, ray-AABB | 중 | - |
...
- [ ] `CubeTextureHandle`과 cube texture cache 추가
- [ ] 논리 이름 하나를 위 6개 파일 경로로 해석하는 `LoadCubeTexture`
- [ ] 여섯 이미지가 정사각형이고 크기·포맷이 같은지 검증
- [ ] `DepthOrArraySize = 6`인 texture resource 생성
- [ ] `GetCopyableFootprints`로 6개 서브리소스 업로드
- [ ] `D3D12_SRV_DIMENSION_TEXTURECUBE` SRV 생성
- [ ] 방향을 확인할 색상·문자 레이블 테스트 큐브맵 에셋 추가
- [ ] Asset Browser가 유효한 `Skyboxes/<name>` 디렉터리를 논리 에셋 하나로 표시
- [ ] skybox 전용 VS/PS, root signature 또는 scene signature 조합 추가
- [ ] translation을 제거한 camera view와 far-depth 출력
- [ ] skybox PSO: depth `LESS_EQUAL`, depth write OFF, 내부 면이 보이는 cull 설정
- [ ] **씬 포맷 버전 정책 선행 작업** (Phase 11에서 포맷을 처음 바꾸는 단계다)
...
- [ ] `Environment`에 선택적 skybox 필드 추가
- [ ] Inspector 선택과 씬 저장/불러오기 지원
...
- [ ] 뷰포트 클릭으로 가장 가까운 기존 메시 엔티티를 선택할 수 있음
- [ ] 새 메시가 로컬 AABB 기준으로 바닥 위에 정확히 배치됨
- [ ] 6장 PNG가 cube texture로 업로드되고 스카이박스로 표시됨
- [ ] 카메라 이동에는 고정되고 회전에는 반응하는 스카이박스
