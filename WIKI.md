...
마우스와 키보드 모두 **커서가 Scene 패널 위에 있을 때만** 동작한다. 다른
패널 위에서는 무시된다. 예외는 텍스트 입력 중일 때로, 이름 칸에 타이핑하는
동안에는 `W`·`S`가 글자로 들어가고 카메라는 움직이지 않는다.
...
커서를 Scene 패널 위에 둔다 — 마우스와 키보드 모두 그때만 동작한다. 이름 칸
같은 텍스트 입력이 활성이면 글자 키는 그쪽으로 간다. 마우스 회전은 오른쪽
버튼을 누른 상태에서 수행한다.

패널을 클릭한 뒤 키가 안 먹는 증상은 해결됐다. 예전에는 ImGui 창에 포커스가
있기만 해도 키보드를 통째로 넘겨받아, 아무 패널도 없는 빈 픽셀을 클릭해 포커스를
풀어야 카메라가 움직였다. 지금은 텍스트 입력 중일 때만 양보한다.
