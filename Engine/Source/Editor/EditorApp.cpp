    ...
    , m_packageBuilder(runtimePaths)
{
    ...
    // The overlay binds the SRV heap itself, so it has to be told when the
    // renderer replaces it. Safe from inside RenderFrame because every id the
    // overlay has already recorded this frame is a logical slot, not an
    // address.
    renderer.SetDescriptorHeapChangedCallback(
        [this]() { m_overlay->RebindDescriptorHeap(); });
